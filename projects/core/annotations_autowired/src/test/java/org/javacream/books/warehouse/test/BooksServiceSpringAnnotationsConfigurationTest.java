...
public class BooksServiceSpringAnnotationsConfigurationTest {
	...
	private IsbnGenerator isbnGenerator;
	 @Autowired @Qualifier(IsbnGenerator.SEQUENCE)
	 private IsbnGenerator isbnGenerator2;

	@Test
	public void testBusinessObjects() {
		TestActor.doTest(booksService);
		Assert.assertFalse(isbnGenerator == isbnGenerator2);
		 // GRÖBSTES HACKING: ((MapBooksService)booksService) NEVER AGAIN!!!!
		 Assert.assertTrue(isbnGenerator == ((MapBooksService)
		 booksService).getIsbnGenerator());
